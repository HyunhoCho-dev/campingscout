...
export default async function SharedTripPage({ params }: { params: Promise<{ shareToken: string }> }) {
  ...
  return <main className="share-page"><section><div className="share-brand"><TentTree size={25} /> CampingScout</div><span className="eyebrow">Shared adventure</span><h1>{trip.title}</h1><p>{camp?.name || "Camping trip"} · {camp?.area || "Korea"}</p><div className="shared-facts"><div><MapPin /><strong>{camp?.landscape || "Outdoor campground"}</strong></div><div><Route /><strong>{camp?.driveMinutes || 120} min drive</strong></div><div><ShieldCheck /><strong>Verify live conditions</strong></div></div><p className="share-note">CampingScout separates public-data facts from AI recommendations. Always confirm operator policies and official alerts before departure.</p><Link href="/">Build my own trip</Link></section></main>;
}

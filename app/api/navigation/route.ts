...
export async function POST(request: NextRequest) {
  const { origin, destination, waypoints = [], minutes = 120 } = await request.json() as { origin?: unknown; destination?: unknown; waypoints?: unknown; minutes?: number };
  ...
  const safeWaypoints = Array.isArray(waypoints) ? waypoints.filter(validCoordinate).slice(0, 5) : [];
  const safeMinutes = Math.max(15, Math.min(240, Number(minutes) || 120));
  const token = process.env.MAPBOX_ACCESS_TOKEN;
  if (!token) return NextResponse.json(await fetchOsrmOrFallback(origin, destination, safeMinutes, safeWaypoints));
  try {
    const mapboxCoordinates = [origin, ...safeWaypoints, destination].map((coordinate) => coordinate.join(",")).join(";");
    const directionsUrl = `https://api.mapbox.com/directions/v5/mapbox/driving/${mapboxCoordinates}?geometries=geojson&overview=full&access_token=${encodeURIComponent(token)}`;
    ...
    return NextResponse.json(await fetchOsrmOrFallback(origin, destination, safeMinutes, safeWaypoints));
  }
}

async function fetchOsrmOrFallback(origin: [number, number], destination: [number, number], minutes: number, waypoints: [number, number][] = []) {
  try {
    const coordinates = [origin, ...waypoints, destination].map((coordinate) => coordinate.join(",")).join(";");
    const url = `https://router.project-osrm.org/route/v1/driving/${coordinates}?overview=full&geometries=geojson`;
    const response = await fetch(url, { headers: { "User-Agent": "CamperLife/1.0" }, next: { revalidate: 3600 } });

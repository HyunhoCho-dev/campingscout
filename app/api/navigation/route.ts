...
export async function POST(request: NextRequest) {
  ...
  if (!token) return NextResponse.json(fallbackNavigation(origin, destination, safeMinutes));
  try {
    ...
    return NextResponse.json(fallbackNavigation(origin, destination, safeMinutes));
  }
}
...
function fallbackNavigation(origin: [number, number], destination: [number, number], minutes: number) { return { route: { type: "LineString", coordinates: [origin, [(origin[0] + destination[0]) / 2, (origin[1] + destination[1]) / 2], destination] }, reach: null, driveMinutes: Math.round(minutes), distanceKm: null, source: "estimated-demo", live: false }; }

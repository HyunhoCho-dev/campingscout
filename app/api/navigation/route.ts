...
export async function POST(request: NextRequest) {
  ...
  const safeWaypoints = Array.isArray(waypoints)
    ? waypoints.filter(validCoordinate).slice(0, 7)
    : [];
  const safeMinutes = Math.max(15, Math.min(480, Number(minutes) || 120));
  const token = process.env.MAPBOX_ACCESS_TOKEN;
  if (!token)
    return NextResponse.json(
      await fetchOsrmOrFallback(
        origin,
        destination,
        safeMinutes,
        safeWaypoints,
  ...
  try {
    const mapboxCoordinates = (
      roundTrip
        ? [origin, destination, ...safeWaypoints, origin]
    ...
    const [directionsResponse, isochroneResponse] = await Promise.all([
      fetch(directionsUrl),
      fetch(isochroneUrl),
    ]);
    if (!directionsResponse.ok || !isochroneResponse.ok)
      throw new Error("Mapbox navigation failed");
    const [directions, isochrone] = (await Promise.all([
      directionsResponse.json(),
      isochroneResponse.json(),
    ])) as [
      {
        routes?: Array<{
          geometry: LineString;
      ...
      { features?: Array<{ geometry: Polygon }> },
    ];
    const route = directions.routes?.[0];
    return NextResponse.json({
      route: route?.geometry,
      distanceKm: route ? Math.round(route.distance / 1000) : null,
      driveMinutes: route ? Math.round(route.duration / 60) : null,
      reach: isochrone.features?.[0]?.geometry,
      source: "mapbox",
      live: true,
    });
  } catch (error) {
...
async function fetchOsrmOrFallback(
  ...
  try {
    const coordinates = (
      roundTrip
        ? [origin, destination, ...waypoints, origin]
